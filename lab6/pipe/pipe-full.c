...
long long gen_f_pc()
{
    return (((((((((id_ex_curr->srca) == (id_ex_curr->srcb)) & (
                    (id_ex_curr->vala) > 0)) & ((id_ex_curr->icode) == 
                  (I_ALU))) & ((id_ex_curr->ifun) == 2)) & (
              (if_id_curr->icode) == (I_JMP))) & ((if_id_next->ifun) == 1))
         & ((id_ex_curr->vala) > 0)) ? (if_id_curr->valp) : ((
...
long long gen_D_bubble()
{
    return (((((id_ex_curr->icode) == (I_JMP)) & !(ex_mem_next->takebranch)
          ) & !(((((id_ex_curr->ifun) == 1) & ((ex_mem_curr->icode) == 
                (I_ALU))) & ((ex_mem_curr->icode) == 2)) & (
            (ex_mem_curr->vale) > 0))) | (!(((id_ex_curr->icode) == 
