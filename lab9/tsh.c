...
void eval(char *cmdline) 
{
    ...
    if(arg[0]==NULL) return; /* Ignore empty lines. */
    ...
    if(!builtin_cmd(arg)){ 
        ...
        sigprocmask(SIG_SETMASK, &prev_all, NULL); //unblock
        if(!bg){ // fg
            waitfg(pid); // wait for the foreground process to end
        }
        else{ //bg
...
int builtin_cmd(char **argv) 
{
    if(!strcmp(argv[0], "quit")) {
        ...
        // switching jobs from bg to fg or fg to bg
        do_bgfg(argv);
        return 1;
    }
