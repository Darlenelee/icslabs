...
# 1. implemented iaddq
# 2. optimize andq & jle data hazard by adding a checking process into f_pc
#/* $begin pipe-all-hcl */
####################################################################
...
word f_pc = [
	E_srcA==E_srcB &&E_valA >0  && E_icode == IOPQ && E_ifun == 2 && D_icode == IJXX && f_ifun == 1&& E_valA >0 :D_valP;
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd &&
	!(E_ifun == 1 && M_icode==IOPQ&& M_icode==2 &&M_valE>0)
	)
	 ||
	# Stalling at fetch while ret passes through pipeline
...
bool E_stall = 0;
bool E_bubble =
#	(M_icode == IJXX && !M_Cnd && E_icode == IOPQ && E_ifun == 0 && D_icode == IOPQ) ||
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd) ||
	# Conditions for a load/use hazard
	E_icode in { IMRMOVQ, IPOPQ } &&
