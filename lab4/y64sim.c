...
long_t compute_alu(alu_t op, long_t argA, long_t argB)
{
    long_t val = 0;
    switch (op){
        case 0: val = argA + argB; break;
        case 1: val = argB - argA; break;
        case 2: val = argA & argB; break;
        case 3: val = argA ^ argB; break;
        default: break;
    }
    return val;
}
...
stat_t nexti(y64sim_t *sim)
{
    ...
    switch(icode){
        case I_RRMOVQ: case I_IRMOVQ: case I_RMMOVQ: case I_MRMOVQ: case I_ALU: case I_POPQ: case I_PUSHQ:
    ...
    long_t imm = 0, argA = 0, argB = 0, val = 0, rsp = 0;
    switch(icode){
    ...
    switch (icode) {
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        argA = get_reg_val(sim->r, regA);   //long
        argB = get_reg_val(sim->r, regB);   //long
        argB += imm;
        set_long_val(sim->m, argB, argA);
        sim->pc = next_pc;
        break;
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        argB = get_reg_val(sim->r, regB); 
        argB += imm;
        if (!get_long_val(sim->m, argB, &val)) {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, argB);
            return STAT_ADR;
        }
      ...
      case I_PUSHQ: /* A:0 regA:F */
        ...
        argA = get_reg_val(sim->r, regA);
        set_reg_val(sim->r, REG_RSP, rsp);
        if (!set_long_val(sim->m, rsp, argA)) {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rsp);
            return STAT_ADR;
        }
        sim->pc = next_pc;
      ...
      case I_POPQ: /* B:0 regA:F */
        rsp = get_reg_val(sim->r, REG_RSP);
        if (!get_long_val(sim->m, rsp, &argA)) {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rsp);
            return STAT_ADR;
        }
        set_reg_val(sim->r, REG_RSP, rsp + 8);
        set_reg_val(sim->r, regA, argA);
